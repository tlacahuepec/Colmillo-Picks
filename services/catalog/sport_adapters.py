...
class SoccerCatalogAdapter:
    ...
    def discover(self, run_date: str) -> list[CatalogEvent]:
        result = self._schedule_provider.fetch(ProviderRequest(
            sport="soccer", resource="schedule", requested_at=self._clock(), date=run_date,
        ))
        ...
        for raw in result.facts.get("matches", []):
            ...
            event, _ = normalize_event(
                raw=raw, sport="soccer", league=str(raw.get("league") or "soccer"),
    ...
    def collect(self, event: CatalogEvent) -> dict[str, ProviderResult]:
        ...
        for resource in SOCCER_CATALOG_CONFIG.resources:
            provider = self._providers.get(resource)
            if provider is None:
                continue
            collected[resource] = provider.fetch(ProviderRequest(
                sport="soccer", resource=resource, requested_at=self._clock(), event_id=event.event_id,
            ))
        return collected
...
def _lineups(event: CatalogEvent, facts: Mapping[str, Any], result: ProviderResult) -> list[LineupSnapshot]:
    ...
    for item in facts.get("lineups", []):
        ...
        players = tuple(canonical_ref("soccer", "player", provider_id=str(player.get("id")) if isinstance(player, Mapping) and player.get("id") else None, display_name=str(player.get("name") if isinstance(player, Mapping) else player)) for player in item.get("players", []))
...
def _injuries(event: CatalogEvent, facts: Mapping[str, Any], result: ProviderResult) -> list[InjurySnapshot]:
    ...
    for item in facts.get("injuries", []):
        ...
        output.append(InjurySnapshot(
            ...
            subject=canonical_ref("soccer", "player", provider_id=str(item.get("player_id")) if item.get("player_id") else None, display_name=name),
